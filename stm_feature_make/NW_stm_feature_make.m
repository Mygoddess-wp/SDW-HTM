function [feature_grad_two,allFeatureName,alabel,slabel]  = NW_stm_feature_make(xyz, joints, frames, coefficient_all)
    addpath('source/')
    ...
    actionMap('Hold') = 3;
    actionMap('Index') = 4;
    actionMap('SideHand') = 5;
    actionMap('SideIndex') = 6;
    actionMap('Thumb') = 7;
    % load data
    folder = './Dataset/Northwestern_Hand_Gesture/';
    ...
    for folderIndex = 1:length(subfolders)
        ...
        aviFiles = dir(fullfile(currentFolder, '*.avi'));
        aviFiles = aviFiles(~startsWith({aviFiles.name}, '.'));
        dataLength = dataLength + length(aviFiles);
    end
    ...
    for folderIndex = 1:length(subfolders)
        ...
        aviFiles = dir(fullfile(currentFolder, '*.avi'));
        aviFiles = aviFiles(~startsWith({aviFiles.name}, '.'));
        for aviIndex = 1:length(aviFiles)
            sample_num = sample_num + 1;
            aviName = aviFiles(aviIndex).name;
            currentAVIFile = fullfile(currentFolder, aviName);
            pattern = '(\d+)_(\w+)_(\d+)\.avi';
            tokens1 = regexp(aviName, pattern, 'tokens', 'once');
            a = str2double(tokens1{1});
            s = actionMap(tokens1{2});
            alabel = [alabel a];
            slabel = [slabel s];
            temp_data = avi2matrix(currentAVIFile,xyz,joints);
