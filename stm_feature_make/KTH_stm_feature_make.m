function [feature_grad_two,allFeatureName,alabel,slabel] = KTH_stm_feature_make(xyz, joints, frames, coefficient_all)
    addpath('source/')
    ...
    for folderIndex = 1:length(subfolders)
        ...
        for aviIndex = 1:length(aviFiles)
            ...
            matchResult = regexp(aviName, 'person(\d+)_([^_]+)_d', 'tokens', 'once');
            if isempty(matchResult)
                fprintf('Failed to extract information from file: %s\n', aviName);
            end
            actionCategory = matchResult{2};
            a = actionMap(actionCategory);
            s = str2double(matchResult{1});
            alabel = [alabel a];
            slabel = [slabel s];
